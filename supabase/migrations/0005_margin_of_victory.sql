-- ELO v2: winner-only margin-of-victory K boost.
-- Loser's K is unchanged; only the winning team's K is multiplied by
--   1 + max(0, score_diff - 2) / 21
-- capped at 2.0. So 21-19 (diff 2) and tighter games behave like classic
-- ELO; bigger margins reward the winner more without extra-punishing
-- the loser. See src/lib/elo.ts and docs/ELO_CALCULATION.md.
-- Safe to rerun.
