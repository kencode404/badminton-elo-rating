...
| --------------------- | ----- | ---------------------------------------------------------------------- |
| `STARTING_RATING`     | 1000  | Initial rating for any new player, both singles and doubles.           |
...
- **STARTING_RATING = 1000**: New players start in Bronze. Aligned with the visible tier system so the climb feels real from day one.
...
| 2026-05-01 | `PROVISIONAL_GAMES` 10 → 5. UI tier system added (Bronze/Silver/Gold/Diamond/Predator) — pure presentation over rating. Tier brackets aligned to a future 1000 starting rating; current 1200 start lands in Silver intentionally. | Faster onboarding to "established" rating, plus visible progression goals. |
| 2026-05-01 | `STARTING_RATING` 1200 → 1000. Profile column defaults updated via migration; existing rows untouched. | Fresh signups now begin in Bronze, matching the tier brackets that were already in place. |
